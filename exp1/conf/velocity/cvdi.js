		...
		// crossDomain: true,
          ...
          success : function(data) {
			  ...
			  for(var i = 0; i < childConut; i++) {
				  ...
				  if(o.term.length > 20) {
					  nodelabel = o.term.substring(0, 20) + "...";
				  }
				  else {
					  nodelabel = o.term;
				  }
				  var node = {x: pos[0], y: pos[1], term: nodelabel, longterm: o.term, rootnode:root, cui: o.cui, num:i+1, nodeid:nodeid++, shrunk:false, ratio:o.freq/maxf, rippler:R};
				  nodes.push(node);
	...
	jQuery.ajax({
		...
		success : function(data) {
			...
			for(var i = 0; i < childConut; i++) {
			  ...
			  nodesize = o.freq/maxf;
			  if(isNaN(nodesize)) {
				  nodesize = 0.5;
			  }
			  console.log("NS:"+nodesize);


				var node = {x: pos[0], y: pos[1], term:nodelabel, longterm: o.term, rootnode:root, cui: o.cui, num:i+1, nodeid:nodeid++, shrunk:false, ratio:nodesize, rippler:R};
				nodes.push(node);
				// links.push({source:root, target:node, path:false});
...
function paint(rootx, rooty, R) {
	...
	vis.selectAll(".node")
		...
		.on("mouseover", function(d){tooltip.text(d.longterm); return tooltip.style("visibility", "visible");})
